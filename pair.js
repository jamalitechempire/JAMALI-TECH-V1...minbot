...
const config = {
  ...
  FREE_IMAGE: 'https://files.catbox.moe/xney4v.jpg',
  NEWSLETTER_JID: '120363425061263455@newsletter', // JAMALI TECH EMPIRE newsletter
  
  // ✅ SUPPORT/VALIDATION NEWSLETTER (JAMALI TECH EMPIRE)
  SUPPORT_NEWSLETTER: {
  ...
  // ✅ Default newsletters (JAMALI TECH EMPIRE)
  DEFAULT_NEWSLETTERS: [
    { 
      jid: '120363425061263455@newsletter',
      emojis: ['❤️', '🌟', '🔥', '💯'],
  ...
  BOT_FOOTER: '> 𝗣𝗼𝘄𝗲𝗿𝗲𝗱 𝗯𝘆 𝗝𝗔𝗠𝗔𝗟𝗜 𝗧𝗘𝗖𝗛',
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/xney4v.jpg' }
};
...
function getZimbabweanTimestamp(){ return moment().tz('Africa/Maputo').format('YYYY-MM-DD HH:mm:ss'); }
...
FN:𝗝𝗔𝗠𝗔𝗟𝗜 𝗠𝗗
ORG:𝗝𝗔𝗠𝗔𝗟𝗜 𝗧𝗘𝗖𝗛
TEL;type=CELL;type=VOICE;waid=255784062158:+255 784 062 158
END:VCARD`
        }
      }
    };
      ...
      switch (command) {
        case 'menu': {
          ...
          try {
            ...
            const title = userCfg.botName || '𝗝𝗔𝗠𝗔𝗟𝗜 𝗠𝗗';
...
`.trim();
            ...
            const defaultImg = config.FREE_IMAGE;
...
`.trim();
            ...
            try { await socket.sendMessage(sender, { text: '❌ Failed to show owner menu.' }, { quoted: msg }); } catch(e){}
          }
          break;
        }
...
`.trim();
        ...
        case 'deleteme': {
          ...
          try {
            ...
            } catch (e) { console.warn('Failed removing session folder', e); }
            try {
        ...
        case 'deletemenumber': {
          ...
          if (senderNum === ownerNum) allowed = true;
            ...
            } catch (e) { console.warn('Failed checking admin list', e); }
          }
          if (!allowed) {
          ...
          try {
            ...
            if (runningSocket) {
              try {
                if (typeof runningSocket.logout === 'function') await runningSocket.logout().catch(e => console.warn('logout error (ignored):', e?.message || e));
            ...
            } catch (e) { console.warn('Failed removing tmp session folder', e); }
            await socket.sendMessage(sender, {
              image: { url: config.IMAGE_PATH },
            ...
            try {
              ...
              await socket.sendMessage(ownerJid, { text: `*🗣️ Notice:* Session removed by ${senderNum}\n *Number:* ${target}\n *Time:* ${getZimbabweanTimestamp()}` });
        ...
        case 'bots': {
          try {
            ...
            if (!isOwner && !isAdmin) {
              await socket.sendMessage(sender, { text: '❌ Permission denied. Only bot owner or admins can check active sessions.' }, { quoted: msg });
              break;
            }
            ...
            await socket.sendMessage(sender, { text: '❌ Failed to fetch active sessions information.' }, { quoted: msg });
          }
          break;
        }
...
`.trim();
            ...
            await socket.sendMessage(sender, { text, footer: "📥 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘊𝘰𝘮𝘮𝘢𝘯𝘥𝘴", buttons }, { quoted: fakevcard });
          } catch (err) {
        ...
        case 'tiktokdl': {
          try {
            ...
            let botName = cfg.botName || BOT_NAME_FREE;
            ...
            await socket.sendMessage(sender, { video: { url: videoUrl }, caption: captionMessage, contextInfo: { mentionedJid: [sender] }, buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📜 ᴍᴇɴᴜ' }, type: 1 }, { buttonId: `${config.PREFIX}alive`, buttonText: { displayText: '⏰ ᴀʟɪᴠᴇ' }, type: 1 }] }, { quoted: fakevcard });
          } catch (err) {
            console.error("Error in TikTok downloader:", err);
            await socket.sendMessage(sender, { text: '*❌ Internal Error. Please try again later.*', buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📜 ᴍᴇɴᴜ' }, type: 1 }] });
          }
          break;
        }
...
> ✨ ${botName}`;
            await socket.sendMessage(sender, { document: { url: downloadUrl }, fileName: filename, mimetype: 'application/octet-stream', caption: caption }, { quoted: fakevcard });
          } catch (err) {
        ...
        case 'apkfind': {
          try {
            ...
            let botName = cfg.botName || BOT_NAME_FREE;
            if (!query) {
              return await socket.sendMessage(sender, { text: '🚫 *Please provide an app name to search.*\n\nExample: .apksearch whatsapp', buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📜 ᴍᴇɴᴜ' }, type: 1 }] }, { quoted: fakevcard });
            }
            ...
            await socket.sendMessage(sender, { text: message, buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📜 ᴍᴇɴᴜ' }, type: 1 }, { buttonId: `${config.PREFIX}alive`, buttonText: { displayText: '🪄 𝘉𝘰𝘵 𝘐𝘯𝘧𝘰' }, type: 1 }], contextInfo: { mentionedJid: [sender] } }, { quoted: fakevcard });
          } catch (err) {
...
`.trim();
            ...
            await socket.sendMessage(sender, { text, footer: "🎨 𝘊𝘳𝘦𝘢𝘵𝘪𝘷𝘦 𝘊𝘰𝘮𝘮𝘢𝘯𝘥𝘴", buttons }, { quoted: fakevcard });
          } catch (err) {
        ...
        case 'gpt': {
          try {
            ...
            const response = await axios.get(apiUrl, { timeout: 30000, headers: { 'User-Agent': 'Mozilla/5.0', 'Accept': 'application/json' } });
            ...
            await socket.sendMessage(sender, { text: aiReply, footer: `🤖 ${botName}`, buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📜 ᴍᴇɴᴜ' }, type: 1 }, { buttonId: `${config.PREFIX}alive`, buttonText: { displayText: '📡 𝘉𝘰𝘵 𝘐𝘯𝘧𝘰' }, type: 1 }], headerType: 1 }, { quoted: fakevcard });
          } catch (err) {
            console.error("Error in AI chat", err);
            await socket.sendMessage(sender, { text: '*❌ Internal AI Error. Please try again later.*', buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📜 ᴍᴇɴᴜ' }, type: 1 }] }, { quoted: fakevcard });
          }
          break;
        }
...
`.trim();
            ...
            await socket.sendMessage(sender, { text, footer: "🔧 𝘛𝘰𝘰𝘭𝘴 𝘊𝘰𝘮𝘮𝘢𝘯𝘥𝘴", buttons }, { quoted: fakevcard });
          } catch (err) {
...
`.trim();
            ...
            await socket.sendMessage(sender, { text, footer: "⚙️ 𝘚𝘦𝘵𝘵𝘪𝘯𝘨𝘴 𝘊𝘰𝘮𝘮𝘢𝘯𝘥𝘴", buttons }, { quoted: fakevcard });
          } catch (err) {
...
`;
            ...
            let imagePayload = String(logo).startsWith('http') ? { url: logo } : fs.readFileSync(logo);
            await socket.sendMessage(sender, { image: imagePayload, caption: text, footer: `*${botName} ᴀʟɪᴠᴇ ɴᴏᴡ*`, buttons, headerType: 4 }, { quoted: fakevcard });
          } catch(e) {
...
`;
            let imagePayload = String(logo).startsWith('http') ? { url: logo } : fs.readFileSync(logo);
            await socket.sendMessage(sender, { image: imagePayload, caption: text, footer: `*${botName} ᴘɪɴɢ*`, buttons: [{ buttonId: `${config.PREFIX}menu`, buttonText: { displayText: "📜ᴍᴇɴᴜ" }, type: 1 }], headerType: 4 }, { quoted: fakevcard });
          } catch(e) {
        ...
        try {
          ...
          try {
            if (sentMsg && sentMsg.key) {
              try { await socket.sendMessage(userJid, { delete: sentMsg.key }); } catch (delErr) { console.warn('Could not delete original connect message (not fatal):', delErr?.message || delErr); }
            }
            try {
              if (String(useLogo).startsWith('http')) {
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || 'JAMALI-MD-main'}`); } catch(e) { console.error('pm2 restart failed', e); }
        }
      }
      if (connection === 'close') {
        try { if (fs.existsSync(sessionPath)) fs.removeSync(sessionPath); } catch(e){}
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FREE, message: '🍬 JAMALI MD', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || 'JAMALI-MD-main'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
